...
@pytest.mark.parametrize(
    "normalization_enabled,raw_probability,expected_probability,expected_originals,expected_max_error,expected_probability_errors",
    [
        (
            False,
            0.2,
...
def test_probability_normalization_and_debug_data(
    ...
    expected_originals: dict[str, dict[str, float]] | None,
    expected_max_error: float,
    expected_probability_errors: dict[str, float],
) -> None:
    ...
    assert probability_normalizations["stars"].probabilities == pytest.approx({"0": expected_probability, "1": expected_probability})
    ...
    assert debug_data["probability_errors"] == pytest.approx(expected_probability_errors)
