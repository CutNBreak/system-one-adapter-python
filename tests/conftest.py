...
class ReadableJsonSerializer:
    ...
    @classmethod
    def serialize(cls, cassette_dict: dict[str, Any]) -> str:
        ...
        for interaction in readable_cassette_dict["interactions"]:
            ...
            request["body"] = cls._decode_json_container_if_possible(request.get("body"))
            response_body = interaction["response"].get("body")
            if isinstance(response_body, dict):
                response_body["string"] = cls._decode_json_container_if_possible(response_body.get("string"))
        return json.dumps(readable_cassette_dict, ensure_ascii=False, indent=4) + "\n"

    @classmethod
    def deserialize(cls, cassette_string: str) -> dict[str, Any]:
        ...
        for interaction in cassette_dict["interactions"]:
            ...
            response_body = interaction["response"].get("body")
            if isinstance(response_body, dict):
                response_body["string"] = cls._encode_json_container_if_present(response_body.get("string"))
