...
def probability_debug_data(
    ...
    probability_errors = {question_id: error for question_id, error in errors.items() if error > PROBABILITY_TOLERANCE}
    original_probabilities = {
        question_id: probability_normalization.original_probabilities
        for question_id, probability_normalization in probability_normalizations.items()
        if probability_normalization is not None and probability_normalization.original_probabilities is not None
    }
...
def rescale_probabilities(probabilities: dict[str, float]) -> dict[str, float]:
    ...
    return {answer: probability / total for answer, probability in probabilities.items()}
